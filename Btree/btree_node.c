				 ...
				 unsigned short **newKeySizes)
{
	...
	bytes = (node->tree->order - 1) * sizeof(char *);
	MEM_CHECK(*newKeys = (char **)malloc(bytes));
	memset(*newKeys, 0, bytes);

	bytes = (node->tree->order - 1) * sizeof(unsigned short);	
	MEM_CHECK(*newKeySizes = (unsigned short *)malloc(bytes));
	memset(*newKeySizes, 0, bytes);
...
btreeReadNodeBlock(GdbBlock *block, const char *buffer, void *extra)
{
	...
	for (i = 0; i < node->tree->order - 1; i++)
	{
		...
		{
			MEM_CHECK_RET_ZERO(node->keys[i] = (char *)malloc(node->keySizes[i]));
...
btreeCreateNodeBlock(GdbBlock *block, void *extra)
{
	...
	MEM_CHECK_RET_ZERO(node = (BTreeNode *)malloc(sizeof(BTreeNode)));
	memset(node, 0, sizeof(BTreeNode));

	node->tree  = tree;
	node->block = block;

	MEM_CHECK_RET_ZERO(node->children = (offset_t *)malloc(tree->order *
												  sizeof(offset_t)));
	memset(node->children, 0, tree->order * sizeof(offset_t));
	
	
	MEM_CHECK_RET_ZERO(node->keySizes =
			  (unsigned short *)malloc((tree->order - 1) *
									   sizeof(unsigned short)));
	memset(node->keySizes, 0, (tree->order -1)  * sizeof(unsigned short));

	
	MEM_CHECK_RET_ZERO(node->keys = (char **)malloc((tree->order - 1) *
										   sizeof(char *)));
...
btreeDestroyNodeBlock(void *data)
{
	...
	{
		btreeSetError("** WARNING: Dirty node has not been written to disk! Offset: %ld", node->block->offset);
		return;
	}
...
btreeReadNode(BTree *tree, offset_t offset)
{
	...
	if (!__verifyNode((BTreeNode *)block->detail))
	{
		assert(0);
	}

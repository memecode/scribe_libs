 ...
 *
 * $Id: db.h,v 1.14 2002/04/07 18:29:40 chipx86 Exp $
 *
 ...
 */
LIBBTREE GDatabase *gdbOpen(const char *filename, GdbType type, PmAccessMode mode);
 ...
 */
LIBBTREE void gdbClose(GDatabase *db);
 ...
 * @return A GDatabase structure.
 */
LIBBTREE GDatabase *gdbCreate(const char *filename, GdbType type);

/**
 * Destroys a GDatabase structure in memory.
 *
 * Calling gdbClose() will automatically call this to BFree up the
 ...
 * @return NULL
 */
LIBBTREE GDatabase *gdbDestroy(GDatabase *db);
 ...
 */
LIBBTREE GdbStatus gdbAddDataEntry(GDatabase *db, GdbHashTable *table,
						  unsigned short key, const void *data, unsigned short size);
 ...
 * @return The status of the operation.
 */
LIBBTREE GdbStatus gdbAddIndexEntry(GDatabase *db, BTree *tree, const char *key,
						   offset_t offset);
 ...
 * @return The status of the operation.
 */
LIBBTREE GdbStatus gdbAddTree(GDatabase *db, BTree *tree, const char *key,
					 BTree **newTree);
 ...
 * @return The status of the operation.
 */
LIBBTREE GdbStatus gdbAddHashTable(GDatabase *db, BTree *tree, const char *key,

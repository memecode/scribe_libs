 ...
 * 
 * $Id: hashtable.c,v 1.16 2002/06/23 10:28:06 chipx86 Exp $
 *
...
htReadBlock(GdbBlock *block, const char *buffer, void *extra)
{
	...
	MEM_CHECK_RET_ZERO(table = (GdbHashTable *)malloc(sizeof(GdbHashTable)));
	...
	MEM_CHECK_RET_ZERO(table->buckets = (GdbBucket **)malloc(table->bucketCount *
													sizeof(GdbBucket *)));
	memset(table->buckets, 0, table->bucketCount * sizeof(GdbBucket *));

	/* Allocate memory for the bucket item counts. */
	MEM_CHECK_RET_ZERO(table->counts = (unsigned char *)malloc(table->bucketCount));
	...
	for (i = 0; i < table->bucketCount; i++)
	{
		...
		for (j = 0; j < table->counts[i]; j++)
		{
			...
			MEM_CHECK_RET_ZERO(bucket = (GdbBucket *)malloc(sizeof(GdbBucket)));	
			...
			MEM_CHECK_RET_ZERO(bucket->data = malloc(bucket->size));
...
htAddString(GdbHashTable *table, unsigned short key, const char *value)
{
	...
	htAdd(table, key, value, GDB_HT_STRING, (unsigned short) strlen(value));
}
...
htAddLong(GdbHashTable *table, unsigned short key, long value)
{
	...
	htAdd(table, key, &value, GDB_HT_LONG, sizeof(long));
}
...
htAddOffset(GdbHashTable *table, unsigned short key, offset_t value)
{
	...
	htAdd(table, key, &value, GDB_HT_OFFSET, sizeof(offset_t));
}
...
htGetString(GdbHashTable *table, unsigned short key)
{
	...
	MEM_CHECK_RET_ZERO(value = (char *)malloc(size + 1));

 ...
 * 
 * $Id: btree_header.c,v 1.14 2002/06/23 10:28:05 chipx86 Exp $
 *
...
btreeReadHeader(GdbBlock *block, const char *buffer, void *extra)
{
	...
	MEM_CHECK_RET_ZERO(tree = (BTree *)malloc(sizeof(BTree)));
...
btreeGetRootNode(BTree *tree)
{
	...
	{
		btreeSetError("Error: B+Tree: Unable to read the root node offset.");
		return 0;
	}
...
btreeGetLeftLeaf(BTree *tree)
{
	...
	{
		btreeSetError("Error: B+Tree: Unable to read the left leaf offset.");
		return 0;
	}
...
btreeGetTreeSize(BTree *tree)
{
	...
	{
		btreeSetError("Error: B+Tree: Unable to read the tree size.");
		return 0;
	}
